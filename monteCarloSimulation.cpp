...
main( int argc, char *argv[ ] )
{
	...
	{
		vs[n]  = Ranf(  VMIN,  VMAX ); // initial velocity in meters / second
		ths[n] = Ranf( THMIN, THMAX ); // launch angle in degrees
 		gs[n]  = Ranf(  GMIN,  GMAX ); // ground distance in meters
 		hs[n]  = Ranf(  HMIN,  HMAX ); // cliff height in meters
 		ds[n]  = Ranf(  DMIN,  DMAX ); // distance to castle in meters
	}
	...
	for( int tries = 0; tries < NUMTRIES; tries++ )
	{
		...
		#pragma omp parallel for default(none), shared(vs, ths, gs, hs, ds, stderr),\
				reduction(+:numHits)
		for( int n = 0; n < NUMTRIALS; n++ )
		{
